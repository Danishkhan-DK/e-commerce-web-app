...
const Nav = ({cartCount, cardCount, cartItems}) => {
      ...
      <div className='bar'>
       ...
       <span className='search-icon'><CiSearch /> </span> 
        ...
        <div className='nav-buttons'>
            ...
            {user.username?(
                <button className='login-btn' onClick={()=> navigate('/logprofile')}>Logout</button>
            ):(
           ...
           <span className='cart-area' onClick={() => navigate('/carting')} > Cart: {cartItems.length} </span>
        </div>
        <div className='logname' onClick={() => navigate('/logprofile')} ><h3>{user.username}</h3></div>
      </div>
